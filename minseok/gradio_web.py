...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    ...
    # -- 예시 질문 버튼 영역 (명시적 생성!) --
    with gr.Row():
        gr.Markdown("질문 예시 :", elem_id="example-title")
        btn_cancer   = gr.Button("🧬 암 보험")
        btn_accident = gr.Button("🚑 상해 보험")
        btn_medical  = gr.Button("💊 실비 보험")
        btn_fire     = gr.Button("🔥 화재 보험")

    btn_cancer.click(
        lambda _: gr.update(value="통합전이암진단비의 보장범위는 어떻게 돼?"),
        inputs=[chat_input], outputs=[chat_input], show_progress=False)
    btn_accident.click(
        lambda _: gr.update(value="교통사고처리지원금의 1인당 한도가 어떻게 돼?"),
        inputs=[chat_input], outputs=[chat_input], show_progress=False)
    btn_medical.click(
        lambda _: gr.update(value="실비보험에 가입했는데, 갱신년도별 보험료 적용 알려줄래?"),
        inputs=[chat_input], outputs=[chat_input], show_progress=False)
    btn_fire.click(
        lambda _: gr.update(value="화재로 인한 벌금형이 확정 판결되었을 때, 벌금형에 해당하는 금액을 얼마만큼 보상받을 수 있어?"),
        inputs=[chat_input], outputs=[chat_input], show_progress=False)
    # -- 예시 질문 버튼 끝 --
